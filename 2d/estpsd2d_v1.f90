PROGRAM rnd2d_v1
! Now same as estpsd2d_v0_hpc.f90
!OCL INDEPENDENT(out)
...
! Write target PSD
s0(1,1)=0
s0(1,1)=0.25_dp*(s0(2,2)+s0(1,2)+s0(2,1)+s0(2,nz))
s0=s0/s0(1,1)
!rint *, s0(1,2)
s0(1,1)=0.0_dp
call write_direct("tmp_s0.bin", s0, nx/2+1,nz)
!open(unit=100,file="tmp_s0.bin",form='unformatted')
...
print *, "Maximum Iteration Number : ", iter_max
...
!      zg=zg/zg(1,2)
      zg=zg/1000.0_dp;
      ! * Write data
      !   write(fname,'("tmp_zg",i5.5,".dat")') ireal
      !   open(100,file=fname)
      !   write(100,'(D14.8)') ((zg(i,j),j=1,nz),i=1,nx)
      !   close(100)
      !   write(fname,'("tmp_zb",i5.5,".dat")') ireal
      !   open(100,file=fname)
      !   write(100,'(D14.8)') ((zb(i,j),j=1,nz),i=1,nx)
      !   close(100)
      !   write(fname,'("tmp_sb",i5.5,".dat")') ireal
      !   open(100,file=fname)
      !   write(100,'(D14.8)') ((sb(i,j)/sb(2,2),j=1,nz),i=1,nx/2+1)
      !   close(100)
      !   write(fname,'("tmp_sg",i5.5,".dat")') ireal
      !   open(100,file=fname)
...
!      if (mod(ireal,10)==0) then
!         write(fname,'("sbe",i3.3,".bin")') ireal
...
!   print *, 'write_data'
!   write(fname,'("zge",i2.2,".dat")') iter
!   open(100,file=fname)
!   write(100,'(D14.8)') ((zge(i,j)/real(nreal),j=1,nz),i=1,nx)
!   close(100)
!   write(fname,'("zbe",i2.2,".bin")') iter
!   open(unit=100,file=fname,form='unformatted')
!   write(100) ((zbe(i,j)/real(nreal),j=1,nz),i=1,nx)
!   close(100)

   write(fname,'("sg",i2.2,".bin")') iter
   call write_direct(fname,sg0,nx/2+1,nz)
!   open(unit=100,file=fname,form='unformatted',access="direct",&
!        & recl=8*nz*(nx/2+1))
!   write(100,rec=1) ((sg0(i,j), j=1,nz),i=1,nx/2+1)
!   close(100)
   write(fname,'("sbe",i2.2,".bin")') iter
   call write_direct(fname,sbe,nx/2+1,nz)
!   open(unit=100,file=fname,form='unformatted',access="direct",&
!        & recl=8*nz*(nx/2+1))
!   write(100,rec=1) ((sbe(i,j)/sbe(1,1),j=1,nz),i=1,nx/2+1)
!   close(100)
