MODULE nrutil
  ...
  !zroots unity
  !routines for houterh operations on vectors
  ...
  !Each NPAR2 must be le the corresponding NPAR
  ...
  END INTERFACE
  !Routines returning a location as an integer value (ifirstloc, 
  !iminloc are not currently overloaded and so do not have a generic 
  !interface here):
  INTERFACE imaxloc
     MODULE PROCEDURE imaxloc_r,imaxloc_i
  END INTERFACE
  !Routines for argument checking and error handling (nrerror is 
  ...
  END INTERFACE
  !Routines relating to polynomials and recurrences (cumprod, zroots 
  !unity are not currently overloaded):
  INTERFACE arth
  ...
  !outerdiv are not currently overloaded):
  INTERFACE outerprod
  ...
  END INTERFACE
  !Routines for skewop erations on matrices (unit matrix, lower 
  !triangle, upper triangle not currently overloaded):
  INTERFACE diagadd
...
CONTAINS
 ...
 FUNCTION reallocate_rv(p,n)
   ...
   if (ierr /= 0) call &
        nrerror(freallocate_rv: problem in attempt to allocate memoryf)
 ...
 FUNCTION reallocate_iv(p,n)
   ...
   if (ierr /= 0) call &
        nrerror(freallocate_iv: problem in attempt to allocate memoryf)
   if (.not. associated(p)) RETURN
   nold=size(p)
 ...
 FUNCTION reallocate_hv(p,n)
   ...
   if (ierr /= 0) call &
        nrerror(freallocate_hv: problem in attempt to allocate memoryf)
   if (.not. associated(p)) RETURN
   nold=size(p)
 ...
 FUNCTION reallocate_rm(p,n,m)
   ...
   if (ierr /= 0) call &
        nrerror(freallocate_rm: problem in attempt to allocate memoryf)
   if (.not. associated(p)) RETURN
   nold=size(p,1)
   mold=size(p,2)
 ...
 FUNCTION reallocate_im(p,n,m)
   ...
   if (ierr /= 0) call &
        nrerror(freallocate_im: problem in attempt to allocate memoryf)
   if (.not. associated(p)) RETURN
   nold=size(p,1)
   mold=size(p,2)
 ...
 SUBROUTINE assert1(n1,string)
   ...
   if (.not. n1) then
      write (*,*) fnrerror: an assertion failed with this tag:f, &
           string
      STOP fprogram terminated by assert1f
   end if
 ...
 SUBROUTINE assert2(n1,n2,string)
   ...
   if (.not. (n1 .and. n2)) then
      write (*,*) fnrerror: an assertion failed with this tag:f, &
           string
      STOP fprogram terminated by assert2f
   end if
 ...
 SUBROUTINE assert3(n1,n2,n3,string)
   ...
   if (.not. (n1 .and. n2 .and. n3)) then
      write (*,*) fnrerror: an assertion failed with this tag:f, &
           string
      STOP fprogram terminated by assert3f
   end if
 ...
 SUBROUTINE assert4(n1,n2,n3,n4,string)
   ...
   if (.not. (n1 .and. n2 .and. n3 .and. n4)) then
      write (*,*) fnrerror: an assertion failed with this tag:f, &
           string
      STOP fprogram terminated by assert4f
   end if
 ...
 SUBROUTINE assert_v(n,string)
   ...
   if (.not. all(n)) then
      write (*,*) fnrerror: an assertion failed with this tag:f, &
           string
      STOP fprogram terminated by assert_vf
   end if
 ...
 FUNCTION assert_eq2(n1,n2,string)
   ...
   if (n1 == n2) then
      ...
      write (*,*) fnrerror: an assert_eq failed with this tag:f, &
           string
      STOP fprogram terminated by assert_eq2f
   end if
 ...
 FUNCTION assert_eq3(n1,n2,n3,string)
   ...
   if (n1 == n2 .and. n2 == n3) then
      ...
      write (*,*) fnrerror: an assert_eq failed with this tag:f, &
           string
      STOP fprogram terminated by assert_eq3f
   end if
 ...
 FUNCTION assert_eq4(n1,n2,n3,n4,string)
   ...
   if (n1 == n2 .and. n2 == n3 .and. n3 == n4) then
      ...
      write (*,*) fnrerror: an assert_eq failed with this tag:f, &
           string
      STOP fprogram terminated by assert_eq4f
   end if
 ...
 FUNCTION assert_eqn(nn,string)
   ...
   if (all(nn(2:) == nn(1))) then
      ...
      write (*,*) fnrerror: an assert_eq failed with this tag:f, &
           string
      STOP fprogram terminated by assert_eqnf
   end if
 END FUNCTION assert_eqn
 SUBROUTINE nrerror(string)
   !Report a message, then die.
   CHARACTER(LEN=*), INTENT(IN) :: string
   write (*,*) fnrerror: f,string
   STOP fprogram terminated by nrerrorf
 ...
 FUNCTION arth_r(first,increment,n)
   ...
   FUNCTION zroots_unity(n,nn)
     ...
     END FUNCTION returning
     !Routines for gouterh operations on vectors. The order convention 
     ...
     SUBROUTINE scatter_add_r(dest,source,dest_index)
       ...
       INTEGER(I4B) :: m,n,j,i
       n=assert_eq2(size(source),size(dest_index),fscatter_add_rf)
       m=size(dest)
       do j=1,n
          i=dest_index(j)
     ...
     SUBROUTINE scatter_add_d(dest,source,dest_index)
       ...
       INTEGER(I4B), DIMENSION(:), INTENT(IN) :: dest_index
       INTEGER(I4B) :: m,n,j,i
       n=assert_eq2(size(source),size(dest_index),fscatter_add_df)
       m=size(dest)
       do j=1,n
          i=dest_index(j)
          if (i > 0 .and. i <= m) dest(i)=dest(i)+source(j)
     ...
     SUBROUTINE scatter_max_r(dest,source,dest_index)
       ...
       INTEGER(I4B), DIMENSION(:), INTENT(IN) :: dest_index
       INTEGER(I4B) :: m,n,j,i
       n=assert_eq2(size(source),size(dest_index),fscatter_max_rf)
       m=size(dest)
       do j=1,n
          i=dest_index(j)
     ...
     SUBROUTINE scatter_max_d(dest,source,dest_index)
       ...
       INTEGER(I4B), DIMENSION(:), INTENT(IN) :: dest_index
       INTEGER(I4B) :: m,n,j,i
       n=assert_eq2(size(source),size(dest_index),fscatter_max_df)
       m=size(dest)
       do j=1,n
          i=dest_index(j)
          if (i > 0 .and. i <= m) dest(i)=max(dest(i),source(j))
     ...
     SUBROUTINE diagadd_rv(mat,diag)
       ...
       INTEGER(I4B) :: j,n
       n = assert_eq2(size(diag),min(size(mat,1),size(mat,2)),fdiagadd_rvf)
       do j=1,n
     ...
     SUBROUTINE diagmult_rv(mat,diag)
       ...
       REAL(SP), DIMENSION(:), INTENT(IN) :: diag
       INTEGER(I4B) :: j,n
       n = assert_eq2(size(diag),min(size(mat,1),size(mat,2)),fdiagmult_rvf)
       do j=1,n
          mat(j,j)=mat(j,j)*diag(j)
       end do
     ...
     FUNCTION get_diag_rv(mat)
       ...
       INTEGER(I4B) :: j
       j=assert_eq2(size(mat,1),size(mat,2),fget_diag_rvf)
       do j=1,size(mat,1)
     ...
     FUNCTION get_diag_dv(mat)
       ...
       INTEGER(I4B) :: j
       j=assert_eq2(size(mat,1),size(mat,2),fget_diag_dvf)
       do j=1,size(mat,1)
     ...
     SUBROUTINE put_diag_rv(diagv,mat)
       ...
       REAL(SP), DIMENSION(:,:), INTENT(INOUT) :: mat
       INTEGER(I4B) :: j,n
       n=assert_eq2(size(diagv),min(size(mat,1),size(mat,2)),fput_diag_rvf)
       do j=1,n
          mat(j,j)=diagv(j)
       end do
