...
!==================================================================
! FOUR1_SP/DP(DATA,ISIGN)
!   1-D Fourier Transform
!==================================================================
! >>Description<<
!
! DATA : n-complex valued array
!  [Time domain]
!    data(1:n)     : 0 =< t <=tn
! 
!  [Frequency domain]
!    data(1)         : f=0
!    data(1:n/2)     : 0 < f < fc
!    data(n:n/2+2:-1): -fc < f < 0  
!    data(n/2+1)     : f=fc,-fc
!    
!   NOTE: Real-valued data in time domain is given,
!         in frequency domain, 
!                 data(1), data(n/2+1) is real
!                 data(n:n/2+2:-1)=conjg(1:n/2)            
!
! ISIGN : integer, 1 (FFT) or -1 (IFFT)
! ------------------------------------------------------------------
...
SUBROUTINE four2_sp(data,isign)
  USE nrtype
  USE nr, ONLY : fourrow
  IMPLICIT NONE
  COMPLEX(SPC), DIMENSION(:,:), INTENT(INOUT) :: data
...
END SUBROUTINE four2_sp

SUBROUTINE four2_dp(data,isign)
  USE nrtype
  USE nr, ONLY : fourrow
  IMPLICIT NONE
  COMPLEX(DPC), DIMENSION(:,:), INTENT(INOUT) :: data
  ...
  temp=transpose(data) !Tranpose.
    call fourrow(temp,isign) !Transform in (original) first dimension.
  data=transpose(temp) !Transpose into data.
END SUBROUTINE four2_dp


!========================================================================
! REALFT_SP(DATA,ISIGN,ZDATA)
!     Real Fourier Transform, single prescion.
!=======================================================================
! >>Description<<
! DATA : n-real valued array
!  --Time domain
!    data(1:n)     : 0 =< t <=tn
!  --Frequency domain (if ZDATA is not given
!    data(1)         : f=0
!    data(2)         : f=fc
!    data(3:n)       : 0 < f < fc
!       data(2*i+1)  : real      part 
!       data(2*i+2)  : imaginary part
!    data(n/2+1)     : f=fc,-fc
!    
! ISIGN : integer, 1 (FFT) or -1 (IFFT)
!
! ZDATA : n/2-complex valued array (Frequency domain components)
!    zdata=cmplx(data(2*i-1),data(2*i)
!
!----------------------------------------------------------------------
...
! DATA : n-real valued array
!  --DISTANCE (real)
!    data(1:m,1:n)   : 0 =< x <=xn, 0=< z <=zn
!  --SPECTRUM (complex)
!    spec(1:m/2, 1:n): 0 =< f1 < fc1, 0=< f2 =< fn2
!    speq(1:n)       : f1=fc1,        0=< f2 =< fn2
!    
! ISIGN : integer, 1 (FFT) or -1 (IFFT)
!
!----------------------------------------------------------------------

SUBROUTINE rlft2_sp(data,spec,speq,isign)
...
END SUBROUTINE rlft2_sp

SUBROUTINE rlft2_dp(data,spec,speq,isign)
  USE nrtype; USE nrutil, ONLY : assert,assert_eq
  USE nr, ONLY : four2
  REAL(DP), DIMENSION(:,:), INTENT(INOUT) :: data
  ...
  INTEGER :: i1,j1,nn1,nn2
  REAL(DP) :: theta
  COMPLEX(DPC) :: c1=(0.5_sp,0.0_sp),c2,h1,h2,w
  ...
  call assert(iand((/nn1,nn2/),(/nn1,nn2/)-1)==0, &
       'dimensions must be powers of 2 in rlft2')
  c2=cmplx(0.0_dp,-0.5_sp*isign,kind=spc)
  theta=TWOPI_D/(isign*nn1)
  wp=cmplx(-2.0_dp*sin(0.5_dp*theta)**2,sin(theta),kind=spc)
  if (isign == 1) then !Case of forward transform.
     spec(:,:)=cmplx(data(1:nn1:2,:),data(2:nn1:2,:),kind=spc)
